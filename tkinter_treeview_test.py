...
def open_search_window():
    ...
    def select_song():
        ...
        add_song(selected_song)

    search_window = tk.Toplevel(root)
    search_window.title("Search Lyrics")
    search_window.geometry("640x480")

    search_frame1 = tk.Frame(search_window, relief="solid", bd=2)
    search_frame1.pack(side="left", fill="both", expand=True)

    search_frame2 = tk.Frame(search_window, relief="solid", bd=2)
    ...
    select_btn.pack()
    add_btn = tk.Button(search_frame1, text='DB 가사 등록', command=open_add_window)
    add_btn.pack()
...
def open_add_window():
    ...
    def register_song():
        ...
        if messagebox.askyesno('가사 등록', 'DB에 등록하시겠습니까?'):
            song = [title, lyrics, type, memo]
            insert_song_to_db(song)
        if messagebox.askyesno('가사 등록', '현재 프로그램에 추가 하시겠습니까?'):
            song = ('', title, lyrics, type, memo)
            add_song(song)
...
def save_song():
    ...
    if selected:  # 수정
        ...
        if messagebox.askyesno('저장', '현재 프로그램에 추가 하시겠습니까?'):
            song = ('', title, lyrics, type, memo)
            add_song(song)
...
def generate_ppt():
    subtitle_maker.generate_ppt(songs)
...
add_btn.pack()
search_btn = tk.Button(frame1, text='DB검색', command=open_search_window)
search_btn.pack()
update_btn = tk.Button(frame1, text='수정', command=update_song_btn)
update_btn.pack()
remove_btn = tk.Button(frame1, text='삭제', command=delete_song)
remove_btn.pack()
clear_btn = tk.Button(frame1, text="clear", command=clear_songs)
clear_btn.pack()
